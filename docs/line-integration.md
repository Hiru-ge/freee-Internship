...
既存のグループLINEでのシフト交代連携を維持しながら、Rails + Herokuでの安定したLINE Bot機能を実現。Google Apps Scriptの制約（302リダイレクト問題、タイムアウト問題）を解決し、安定したWebhook処理を提供します。
...
- **Google Apps Scriptの制約**: 302リダイレクト問題、6分タイムアウト制限
- **Webhook処理の不安定性**: レスポンス時間の不安定さ

### 2.2. Rails移行による解決
- **安定したWebhook処理**: Herokuでの安定したLINE Webhook処理
- **タイムアウト問題の解決**: 6分制限の解消
- **既存のLINEでの連携体験を維持**: ユーザー体験は変更なし
- **シフト交代の自動化とシステム連携**: PostgreSQLでの安定したデータ管理
- **手動でのシフト表更新作業の削減**: 自動更新機能の実現
...
- **ホスティング**: Heroku
- **データベース**: PostgreSQL（Heroku標準）
- **バックグラウンド処理**: Sidekiq + Redis
- **LINE Bot SDK**: line-bot-api gem

### 10.2. LINE Bot API連携
- LINE Messaging APIの利用
- Rails WebhookControllerでのエンドポイント実装
- メッセージの送受信処理
- グループ・個人の識別処理
- **安定性**: Herokuでの安定したWebhook処理

### 10.3. 認証・セキュリティ
- LINEアカウントと従業員IDの安全な紐付け（PostgreSQL）
- 環境変数によるアクセストークン管理
- Rails標準のセキュリティ機能
- グループ・個人の適切な権限管理

### 10.4. データ連携
- PostgreSQLでの勤怠管理システム連携
- シフト表の自動更新（データベース直接操作）
- リアルタイムでのデータ同期
- グループ・個人での情報分離
- **パフォーマンス**: インデックス最適化による高速クエリ
...
### 12.1. 技術的制約（Rails移行後）
- **解決済み**: Google Apps Scriptの制約（302リダイレクト、タイムアウト）
...
### 13.2. 日常運用（Rails移行後）
1. グループLINEでのシフト交代連携（安定したWebhook処理）
2. 個人LINEでの勤怠・給与確認（PostgreSQL連携）
3. 自動通知による情報共有（Sidekiqバックグラウンド処理）

### 13.3. メンテナンス（Rails移行後）
1. Herokuでの定期的なデータ同期確認
2. Railsログによるエラー監視
3. PostgreSQLのパフォーマンス監視
4. ユーザーフィードバックの収集
5. 必要に応じた機能改善
