...
class AccessControlController < ApplicationController
  ...
  skip_before_action :require_login, only: %i[index authenticate_email verify_code]
  skip_before_action :set_header_variables, only: %i[index authenticate_email verify_code]

  def index
    # 既にメールアドレス認証済みの場合はホームページにリダイレクト
    return unless session[:email_authenticated]
