class ShiftAdditionService
  ...
  def create_addition_request(params)
    begin
      ...
      if Date.parse(params[:shift_date]) < Date.current
        return { success: false, message: "過去の日付のシフト追加依頼はできません。" }
      end
  ...
  def get_addition_status(employee_id)
    begin
      ...
      if all_requests.empty?
        return { success: true, message: "シフト追加リクエストはありません。" }
      end
  ...
  def validate_addition_params(params)
    required_fields = [:requester_id, :shift_date, :start_time, :end_time, :target_employee_ids]
