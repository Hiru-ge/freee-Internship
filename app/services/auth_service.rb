...
class AuthService
  ...
  def self.login(employee_id, password)
    begin
      ...
      return { success: true, message: 'ログインしました', employee: employee }
    rescue => e
  ...
  def self.send_verification_code(employee_id)
    begin
      ...
      if employee_info.nil? || employee_info[:email].blank?
  ...
  def self.set_initial_password_with_verification(employee_id, password, verification_code)
    begin
      ...
      unless password_result[:success]
  ...
  def self.send_password_reset_code(employee_id)
    begin
      ...
      employee = Employee.find_by(employee_id: employee_id)
      if employee.nil?
        return { success: false, message: '指定された従業員IDが見つかりません' }
      end
  ...
  def self.verify_password_reset_code(employee_id, code)
    begin
      ...
      return { success: true, message: '認証コードが正しく確認されました' }
    rescue => e
