...
class LineBotService
  ...
  def handle_shift_addition_approval_postback(line_user_id, postback_data, action)
    ...
    return "このリクエストを承認する権限がありません。" unless addition_request.target_employee_id == employee.employee_id
  ...
  def handle_approval_postback(line_user_id, postback_data, action)
    ...
    return "シフト交代リクエストが見つかりません。" unless exchange_request

    # 権限チェック（承認者は交代先のシフトの担当者である必要がある）
    employee = Employee.find_by(line_id: line_user_id)
    return "このリクエストを承認する権限がありません。" unless exchange_request.approver_id == employee.employee_id
    ...
    if action == "approve"
      ...
      if shift
        ...
        if existing_shift
          ...
          if shift_fully_contained?(existing_shift, new_shift_data)
            ...
            # 含まれていない場合はマージ
            merge_shifts(existing_shift, new_shift_data)
          end
        else
          # 既存シフトがない場合は新規作成
          Shift.create!(
      ...
      shift = exchange_request.shift
      if shift
        send_approval_notification_to_requester(exchange_request, "rejected", shift.shift_date, shift.start_time,
                                                shift.end_time)
      end
  ...
  def line_bot_client
    @line_bot_client ||= if Rails.env.production?
  ...
  def find_employee_by_line_id(_line_id)
    nil
  end

  def link_employee_to_line(_employee_id, _line_id)
    false
  end

  def unlink_employee_from_line(_line_id)
    false
  end
  ...
  def generate_verification_code_for_line(_line_user_id, _employee_id)
    false
  end

  def valid_employee_id_format?(_employee_id)
    false
  end

  def send_verification_code_via_email(_employee_id, _line_user_id)
    false
  end

  def complete_line_account_linking(_line_user_id, _employee_id, _verification_code)
    false
  end

  def validate_verification_code_for_linking(_employee_id, _verification_code)
    false
  end
  ...
  def get_group_shift_info(_group_id)
    # グループ内の全従業員のシフト情報を取得
    now = Date.current
  ...
  def get_daily_shift_info(_group_id, date)
    # 指定日の全従業員のシフト情報を取得
  ...
  def handle_shift_command(event)
    ...
    unless employee_already_linked?(line_user_id)
      return "認証が必要です。「認証」と入力して認証を行ってください。" unless group_message?(event)
  ...
  def handle_shift_exchange_command(event)
    ...
    set_conversation_state(line_user_id, { step: "waiting_shift_date" }) unless group_message?(event)
  ...
  def handle_request_check_command(event)
    ...
    unless utility_service.employee_already_linked?(line_user_id)
      return "認証が必要です。「認証」と入力して認証を行ってください。" unless utility_service.group_message?(event)
    ...
    return "承認待ちのリクエストはありません" if pending_exchange_requests.empty? && pending_addition_requests.empty?
            ...
            { type: "separator", margin: "md" },
            {
              type: "box", layout: "vertical", margin: "md", spacing: "sm", contents: [
                ...
                },
                {
                  type: "box", layout: "baseline", spacing: "sm", contents: [
                    { type: "text", text: "📅", size: "sm", color: "#666666" },
                    { type: "text", text: "#{shift.shift_date.strftime('%m/%d')} (#{day_of_week})", wrap: true,
                ...
                },
                {
                  type: "box", layout: "baseline", spacing: "sm", contents: [
                    { type: "text", text: "⏰", size: "sm", color: "#666666" },
                    { type: "text", text: "#{shift.start_time.strftime('%H:%M')}-#{shift.end_time.strftime('%H:%M')}",
                      wrap: true, color: "#666666", size: "sm", flex: 0 }
                  ]
                }
              ]
            }
            ...
            { type: "separator", margin: "md" },
            {
              type: "box", layout: "vertical", margin: "md", spacing: "sm", contents: [
                ...
                },
                {
                  type: "box", layout: "baseline", spacing: "sm", contents: [
                    { type: "text", text: "📅", size: "sm", color: "#666666" },
                    { type: "text", text: "#{request.shift_date.strftime('%m/%d')} (#{day_of_week})", wrap: true,
                      color: "#666666", size: "sm", flex: 0 }
                  ]
                },
                {
                  type: "box", layout: "baseline", spacing: "sm", contents: [
                    { type: "text", text: "⏰", size: "sm", color: "#666666" },
                    { type: "text",
                      text: "#{request.start_time.strftime('%H:%M')}-#{request.end_time.strftime('%H:%M')}", wrap: true, color: "#666666", size: "sm", flex: 0 }
                  ]
                }
              ]
            }
  ...
  def handle_auth_command(event)
    ...
    # 既に認証済みかチェック
    return "既に認証済みです。シフト確認などの機能をご利用いただけます。" if employee_already_linked?(line_user_id)
  ...
  def handle_employee_name_input(line_user_id, employee_name)
    ...
    if matches.empty?
      ...
      return nil if employee_name.length > 20 || employee_name.match?(/[^\p{Hiragana}\p{Katakana}\p{Han}\s]/)
      ...
      generate_verification_code_for_employee(line_user_id, employee)
    else
      # 複数件の場合は選択肢を提示
  ...
  def handle_multiple_employee_matches(_line_user_id, employee_name, matches)
  ...
  def handle_verification_code_input(line_user_id, employee_id, verification_code)
    ...
    return "認証コードが正しくありません。正しい認証コードを入力してください。" if verification_record.nil?

    return "認証コードの有効期限が切れています。再度「認証」コマンドから始めてください。" if verification_record.expired?
  ...
  def handle_shift_time_input(line_user_id, message_text, state)
    ...
    if message_text.match?(/^\d{2}:\d{2}-\d{2}:\d{2}$/)
      ...
      return "指定された時間は、全従業員が既にシフトに入っています\n別の時間を選択してください" if available_employees.empty?
  ...
  def handle_employee_selection_input(line_user_id, message_text, state)
    ...
    return selection_result[:error] if selection_result[:error]
  ...
  def parse_employee_selection(message_text)
    ...
    error_messages << "複数の従業員が見つかりました: #{ambiguous_names.join(', ')}\nより具体的な名前を入力してください" if ambiguous_names.any?

    error_messages << "❌ 従業員が見つかりません: #{not_found_names.join(', ')}" if not_found_names.any?

    return { error: error_messages.join("\n"), employee_ids: [] } if error_messages.any?
  ...
  def get_available_employees_for_exchange(shift_date, shift_time)
    ...
    all_employees.each do |employee|
      ...
      next if overlap_result[:has_overlap]
  ...
  def handle_shift_selection_input(line_user_id, message_text)
    ...
    if message_text.match?(/^shift_\d+$/)
      ...
      return "選択されたシフトが見つかりません。もう一度選択してください。" unless shift
      ...
      # 依頼可能な従業員を取得
      available_employees = get_available_employees_for_exchange(shift.shift_date.strftime("%Y-%m-%d"),
                                                                 "#{shift.start_time.strftime('%H:%M')}-#{shift.end_time.strftime('%H:%M')}")
  ...
  def handle_shift_addition_command(event)
    ...
    # オーナー権限チェック
    employee = Employee.find_by(line_id: line_user_id)
    return "シフト追加はオーナーのみが利用可能です。" unless employee&.owner?
  ...
  def handle_shift_addition_employee_input(line_user_id, message_text, state)
    ...
    confirmation_message = "📋 シフト追加依頼の確認\n\n" +
                           "📅 日付: #{Date.parse(state['shift_date']).strftime('%m/%d')} (#{%w[日 月 火 水 木 金
                                                                                                土][Date.parse(state['shift_date']).wday]})\n" \
                           "⏰ 時間: #{state['shift_time']}\n" \
                           "👥 対象: #{available_employees.map(&:display_name).join(', ')}\n\n"

    confirmation_message += overlap_message if overlapping_employees.any?
  ...
  def validate_shift_time(time_text)
    ...
    begin
      ...
      return { error: "開始時間は終了時間より早く設定してください。\n例：09:00-18:00" } if start_time >= end_time
