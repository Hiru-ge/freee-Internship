...
#### Phase 16-7: 最後のリファクタリング
- [ ] Concernと Controller の重複解消
  - 対象: 各種ConcernとController
  - 依存関係: ルーティング見直し後の責務整理
- [ ] バリデーションのモデル移行
  - 原則: モデル（ActiveModelのvalidation）へ集約、コントローラから排除
  - 依存関係: 重複解消の設計決定
- [ ] ルーティング命名のRails慣例順守
  - new/create/update/destroy 等へ整理、loginは sessions へ段階移行
  - 依存関係: コントローラのアクション分割方針
- [ ] サービス内メソッドのモデル移行
  - 対象: ドメインロジック（モデルに属する責務）
  - 依存関係: バリデーション移行
- [ ] ドキュメント整備
  - docs 配下および本ファイルを現状に同期（.md に絵文字は使用しない）
  - 依存関係: 上記各作業の反映
