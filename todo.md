...
### Phase 12: LINE Bot リファクタリング
**期間**: 6-7週間
**優先度**: 🔴 最高
**理由**: 現在のLineBotService（2,294行、92メソッド）は巨大なクラスとなっており、保守性・可読性・テスタビリティの向上が急務

#### Phase 12-1: 責任の分離（優先度：🔴 最高）
**期間**: 2-3週間
**工数**: 16時間

- [ ] 認証関連サービスの分離
  - LineAuthenticationServiceの作成
...
- [ ] 会話状態管理サービスの分離
  - LineConversationServiceの作成
  - 会話状態管理、状態遷移処理、入力検証処理の移行
  - 見積時間: 2時間
  - **技術要件**: 既存の会話状態管理機能の移行
  - **依存関係**: 既存の会話状態管理機能の動作保証

#### Phase 12-2: 共通処理の抽出（優先度：🟡 重要）
**期間**: 1週間
**工数**: 8時間

- [ ] バリデーションサービスの作成
  - LineValidationServiceの作成
...
#### Phase 12-4: コントローラーの簡素化（優先度：🟢 通常）
**期間**: 1週間
**工数**: 4時間

- [ ] LineBotServiceの簡素化
  - 巨大なクラス（2,294行、92メソッド）の簡素化
  - 各サービスクラスへの依存関係の整理
  - ルーティング処理のみに責任を限定
  - 見積時間: 2時間
...
#### 2. 段階的実装
1. **Phase 12-1**: 責任の分離（各サービスを順次作成）
2. **Phase 12-2**: 共通処理の抽出
3. **Phase 12-3**: バックエンド処理の統合
4. **Phase 12-4**: コントローラーの簡素化
...
| フェーズ | 工数 | 期間 | 優先度 |
|---------|------|------|--------|
| Phase 12-1: 責任の分離 | 16時間 | 2-3週間 | 🔴 最高 |
| Phase 12-2: 共通処理の抽出 | 8時間 | 1週間 | 🟡 重要 |
| Phase 12-3: バックエンド処理の統合 | 12時間 | 2週間 | 🟡 重要 |
| Phase 12-4: コントローラーの簡素化 | 4時間 | 1週間 | 🟢 通常 |
| **合計** | **40時間** | **6-7週間** | - |
