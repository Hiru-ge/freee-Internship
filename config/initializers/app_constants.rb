...
# 設定の検証
def validate_config!
  required_keys = %i[wage employee auth email ui system]

  required_keys.each do |key|
    Rails.logger.error "必須の設定キーが見つかりません: #{key}" unless APP_CONSTANTS.key?(key)
  end
end

# 開発環境でのみ設定を検証
validate_config! if Rails.env.development?
